...
<div style="padding: 20px;">
    <div style="background: var(--card-bg); border-radius: 12px; padding: 20px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
            <h5 style="margin: 0;"><i class="fa-solid fa-users-gear"></i> مدیریت اعضای تیم</h5>
            <a href="{{ route('team.create') }}" class="btn btn-sm btn-primary" style="padding: 8px 16px; border-radius: 8px; text-decoration: none;">
                <i class="fa-solid fa-plus"></i> افزودن عضو
            </a>
        </div>
        ...
        <table style="width: 100%; border-collapse: collapse;">
            <thead>
                <tr style="border-bottom: 1px solid var(--border);">
                    ...
                    <th style="padding: 12px;">سمت</th>
                    <th style="padding: 12px;">ترتیب نمایش</th>
            ...
            <tbody>
                @forelse($teamMembers as $i => $member)
                    <tr style="border-bottom: 1px solid var(--border);">
                        ...
                        <td style="padding: 12px;">
                            <a href="{{ route('team.edit', $member->id) }}" class="btn btn-sm btn-warning" style="margin-left:3px;padding:6px 10px;border-radius:6px;text-decoration:none;"><i class="fa-solid fa-edit"></i></a>
                            <form action="{{ route('team.destroy', $member->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('این عضو حذف شود؟')">
                                @csrf @method('DELETE')
                ...
                @empty
                    <tr><td colspan="6" style="text-align:center;padding:40px;">هنوز عضوی ثبت نشده</td></tr>
                @endforelse
            </tbody>
        </table>

        <div style="margin-top:20px;">{{ $teamMembers->links() }}</div>
    </div>
</div>
@endsection
