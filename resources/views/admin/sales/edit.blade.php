            ...
            <form action="{{ route('sales.update', $sale->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-grid form-grid-3">
                    <div class="form-group">
                        ...
                        <input type="text" name="title" value="{{ old('title', $sale->title) }}" required class="form-input"
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="price" value="{{ old('price', $sale->price) }}" class="form-input"
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="number" value="{{ old('number', $sale->number) }}" class="form-input"
                            placeholder="اختیاری">
                        @error('number')
                            <small class="form-error">{{ $message }}</small>
                        @enderror
                ...
                <div class="form-group" style="margin-bottom: 15px;">
                    ...
                    @if($sale->image_url)
                        <div>
                            <small style="color: var(--text-dimmer); display: block; margin-top: 8px;">تصویر فعلی:</small>
                            <img src="{{ asset('storage/' . $sale->image_url) }}" class="current-image" alt="تصویر محصول">
                        </div>
                    @endif
                    @error('image')
                        <small class="form-error">{{ $message }}</small>
                ...
                <div class="form-group" style="margin-bottom: 20px;">
                    ...
                    <textarea name="text" rows="8" class="form-textarea"
                        placeholder="توضیحات محصول...">{{ old('text', $sale->text) }}</textarea>
                    @error('text')
                        <small class="form-error">{{ $message }}</small>
                    @enderror
                </div>
