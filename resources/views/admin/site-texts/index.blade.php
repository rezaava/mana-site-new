        ...
        <form action="{{ route('site-texts.update') }}" method="POST">
            ...
            @foreach($texts as $key => $item)
                <div style="margin-bottom: 18px;">
                    <label style="display: block; margin-bottom: 6px; font-weight: 600; color: var(--accent);">{{ $item['label'] }}</label>
                    <input type="text" name="{{ $key }}" value="{{ \App\Models\SiteText::get($key, $item['value']) }}" style="width: 100%; padding: 10px; border-radius: 8px; border: 1px solid var(--border); background: transparent; color: inherit;">
                </div>
            @endforeach
