        ...
        <form action="{{ route('socials.update', $social->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 15px;">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $social->name) }}" required style="width: 100%; padding: 10px; border-radius: 8px; border: 1px solid var(--border); background: transparent; color: inherit;">
                    @error('name')
                ...
                <div>
                    ...
                    <input type="url" name="url" value="{{ old('url', $social->url) }}" required style="width: 100%; padding: 10px; border-radius: 8px; border: 1px solid var(--border); background: transparent; color: inherit;">
                    @error('url')
                        <small style="color: #ef4444; display: block; margin-top: 4px;">{{ $message }}</small>
                    @enderror
                </div>
            ...
            <div style="margin-bottom: 20px;">
                <label style="display: block; margin-bottom: 8px;">تصویر / آیکون جدید (اختیاری)</label>
