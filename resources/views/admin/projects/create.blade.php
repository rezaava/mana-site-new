...
@section('content')
<style>
    /* استایل‌های مشابه edit.blade.php */
    .form-section-title {
        font-size: 1.1rem;
        font-weight: 700;
        color: var(--accent);
...
<div style="padding:20px;">
    <div style="background:var(--card-bg);border-radius:12px;padding:25px;">
        <h5 style="margin-bottom:25px;font-weight:700;">
            <i class="fa-solid fa-plus-circle" style="color:var(--accent-2);margin-left:10px;"></i> افزودن پروژه جدید
        </h5>
            ...
            <div class="stats-grid">
                @for($i = 0; $i < 4; $i++)
                    <div class="field-group" style="border:1px solid var(--border);border-radius:8px;padding:12px;background:var(--bg-soft);">
                        <span class="field-label" style="font-weight:700;color:var(--text);">آمار {{ $i + 1 }}</span>
                        <div style="margin-top:6px;">
                            <span class="field-label">مقدار</span>
                            <input type="text" name="stats_value[]" value="{{ old('stats_value.'.$i) }}" class="form-control-custom" placeholder="مثلاً ۴۵%">
                        </div>
                        <div style="margin-top:6px;">
                            <span class="field-label">برچسب</span>
                            <input type="text" name="stats_label[]" value="{{ old('stats_label.'.$i) }}" class="form-control-custom" placeholder="مثلاً افزایش نرخ تبدیل">
                        </div>
                    </div>
                @endfor
            </div>
            ...
            @foreach($galleryCategories as $catKey => $catLabel)
                <div class="gallery-category">
                    ...
                    <div class="gallery-files">
                        @for($i = 0; $i < 3; $i++)
                            <div class="file-item">
                                <span class="field-label" style="font-size:0.75rem;color:var(--text-dimmer);">تصویر {{ $i+1 }}</span>
            ...
            <div class="tech-grid">
                @foreach($allTechnologies as $tech)
                    <label style="display:flex;align-items:center;gap:8px;padding:8px 12px;border:1px solid var(--border);border-radius:8px;cursor:pointer;transition:all 0.3s;">
                        <input type="checkbox" name="technologies[]" value="{{ $tech['name'] }}" style="width:18px;height:18px;accent-color:var(--accent-2);cursor:pointer;">
                        <i class="{{ $tech['icon'] }}" style="color:var(--accent-2);font-size:1.2rem;width:24px;"></i>
                        <span style="font-weight:500;">{{ $tech['name'] }}</span>
                    </label>
                @endforeach
            </div>
            <!-- فیلدهای مخفی برای آیکون‌ها -->
            @foreach($allTechnologies as $tech)
            ...
            <div class="services-grid">
                @for($i = 0; $i < 5; $i++)
                    <div class="field-group">
                        ...
                        <input type="text" name="service_name[]" value="{{ old('service_name.'.$i) }}" class="form-control-custom" placeholder="مثلاً طراحی UX/UI">
                    </div>
                @endfor
            </div>
