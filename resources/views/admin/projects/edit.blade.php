    ...
    <div style="padding:20px;">
        <div style="background:var(--card-bg);border-radius:12px;padding:25px;">
            ...
            <form action="{{ route('projects.update', $project->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="subtitle" value="{{ old('subtitle', $project->subtitle) }}"
                            class="form-control-custom" placeholder="مثلاً: افزونه‌ای برای حرفه‌ای شدن">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">دسته‌بندی</label>

                        <select name="cat_id" class="form-control-custom">
                            <option style="color: var(--text-dim);" value="">
                                انتخاب دسته‌بندی...
                            </option>

                            @foreach($categories as $category)
                                <option value="{{ $category->id }}" style="color: var(--text-dim);" @if(old('cat_id', $project->cat_id) == $category->id) selected @endif>
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="client_name" value="{{ old('client_name', $project->client_name) }}"
                            class="form-control-custom" placeholder="مثلاً: شرکت بهین فرتاک">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">سمت کارفرما</label>
                        <input type="text" name="client_role" value="{{ old('client_role', $project->client_role) }}"
                            class="form-control-custom" placeholder="مثلاً: مدیرعامل">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">سال اجرا</label>
                        <input type="text" name="launch_year" value="{{ old('launch_year', $project->launch_year) }}"
                            class="form-control-custom" placeholder="مثلاً: ۱۴۰۲">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">مدت زمان</label>
                        <input type="text" name="duration" value="{{ old('duration', $project->duration) }}"
                            class="form-control-custom" placeholder="مثلاً: ۳ سال">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">لینک پروژه</label>
                        <input type="url" name="project_link" value="{{ old('project_link', $project->project_link) }}"
                            class="form-control-custom" placeholder="https://example.com">
                    </div>
                    <div class="col-12">
                        <label class="form-label">توضیح کوتاه (Brief)</label>
                        <textarea name="brief" rows="2"
                            class="form-control-custom">{{ old('brief', $project->brief) }}</textarea>
                    </div>
                    <div class="col-12">
                        <label class="form-label">توضیحات کامل پروژه</label>
                        <textarea name="desc" rows="4"
                            class="form-control-custom">{{ old('desc', $project->desc) }}</textarea>
                    </div>
                    <div class="col-12">
                        <label class="form-label">هدف پروژه</label>
                        <textarea name="project_goal" rows="3" class="form-control-custom"
                            placeholder="هدف از انجام این پروژه چه بود؟">{{ old('project_goal', $project->project_goal) }}</textarea>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label" style="color:#f5a623;"><i class="fa-solid fa-triangle-exclamation"></i>
                            چالش اصلی</label>
                        <textarea name="challenge" rows="4" class="form-control-custom"
                            placeholder="چالش‌های اصلی پروژه را بنویسید...">{{ old('challenge', $project->challenge) }}</textarea>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label" style="color:#00d1b2;"><i class="fa-solid fa-lightbulb"></i> راه‌حل
                            ما</label>
                        <textarea name="solution" rows="4" class="form-control-custom"
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="testimonial" rows="3" class="form-control-custom"
                        ...
                        <div class="file-input-wrapper">
                            ...
                            @if($project->image_url)
                                <div class="existing-image-preview">
                                    ...
                                    <span style="font-size:0.85rem;color:var(--text-dimmer);align-self:center;">تصویر
                                        فعلی</span>
                                </div>
                            @endif
                        </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="slug" value="{{ old('slug', $project->slug) }}" class="form-control-custom"
                            placeholder="slug">
                    </div>
                ...
                <p class="section-description">حداکثر ۴ آیتم آمار را وارد کنید. هر آیتم شامل یک مقدار و یک برچسب توضیحی است.
                </p>
                <div class="stats-grid">
                    ...
                    @foreach($stats as $index => $stat)
                        <div class="field-group"
                            style="border:1px solid var(--border);border-radius:8px;padding:12px;background:var(--bg-soft);">
                            <span class="field-label" style="font-weight:700;color:var(--text);">آمار {{ $index + 1 }}</span>
                            <div style="margin-top:6px;">
                                <span class="field-label">مقدار</span>
                                <input type="text" name="stats_value[]" value="{{ $stat['value'] ?? '' }}"
                                    class="form-control-custom" placeholder="مثلاً ۴۵%">
                            </div>
                            <div style="margin-top:6px;">
                                <span class="field-label">برچسب</span>
                                <input type="text" name="stats_label[]" value="{{ $stat['label'] ?? '' }}"
                                    class="form-control-custom" placeholder="مثلاً افزایش نرخ تبدیل">
                            </div>
                        </div>
                    @endforeach
                </div>
        ...
        <p class="section-description">هر دسته‌بندی می‌تواند تا ۳ تصویر داشته باشد. تصاویر جدید جایگزین تصاویر قبلی می‌شوند.
        </p>
        @php
            ...
            <div class="gallery-category">
                ...
                <div class="gallery-files">
                    ...
                    @foreach($images as $idx => $img)
                        <div class="file-item">
                            <span class="field-label" style="font-size:0.75rem;color:var(--text-dimmer);">تصویر
                                {{ $idx + 1 }}</span>
        ...
        <div class="tech-grid" style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-top:10px;">
            @foreach($allTechnologies as $tech)
                <label
                    style="display:flex;align-items:center;gap:8px;padding:8px 12px;border:1px solid var(--border);border-radius:8px;cursor:pointer;transition:all 0.3s;background:{{ in_array($tech['name'], $projectTechNames) ? 'var(--surface-2)' : 'transparent' }};">
                    <input type="checkbox" name="technologies[]" value="{{ $tech['name'] }}" {{ in_array($tech['name'], $projectTechNames) ? 'checked' : '' }}
        ...
        <div class="services-grid">
            ...
            @foreach($services as $service)
                <div class="field-group">
                    ...
                    <input type="text" name="service_name[]" value="{{ $service['name'] ?? '' }}" class="form-control-custom"
